...
export function getSingleNetworkCondition(stat, yellowThreshold, redThreshold) {
    ...
    if (stat >= redThreshold)
        return NetworkCondition.Red;
    if (stat >= yellowThreshold)
        return NetworkCondition.Yellow;
    return NetworkCondition.Green;
}
export default function getNetworkCondition(testReport) {
    ...
    return Math.min(getSingleNetworkCondition(latency, 200, 400), getSingleNetworkCondition(jitter, 30, 100), getSingleNetworkCondition(packetLoss, 3, 7));
}
